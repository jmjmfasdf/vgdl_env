...
from vgdl.ontology import MovingAvatar, HorizontalAvatar, VerticalAvatar, FlakAvatar, AimedFlakAvatar, OrientedAvatar, \
...
class WBP():
	def __init__(self, rle, gameFilename, theory=None, fakeInteractionRules = [], seen_limits=[], annealing=1, max_nodes=100000, shortHorizon=False,
		...
		self.pixel_size = max(1, self.rle._game.screensize[0]/self.rle._game.width)
		# Ensure dimensions are positive by using maximum of 1 for each dimension
		dimensions = np.maximum(np.ones(2), np.array(self.rle._game.screensize)/self.pixel_size)
		self.visited_positions = np.zeros(dimensions.astype(int))
...
## multiple samples??
	...
	def spritecounter_val(self, theory, term, stype, rle, first_alpha=10000.,
		...
		if compute_second_order:
			...
			try:
				if len(objs)>0:
					# Find non-empty object lists with maximum length
					non_empty_objs = [o for o in objs if len(o)>0]
					if non_empty_objs:
						max_len = max([len(obj) for obj in non_empty_objs])
						valid_objs = [o for o in non_empty_objs if len(o)==max_len]
						if valid_objs:
							kill_positions = np.concatenate(valid_objs)
						else:
							kill_positions = np.array([], dtype=int)
					else:
						kill_positions = np.array([], dtype=int)
				else:
					kill_positions = np.array([], dtype=int)
