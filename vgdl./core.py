...
# Import only what's needed to avoid circular imports
# We'll import specific classes from ontology as needed in methods
import os, shutil
import datetime
import uuid
import subprocess
...
class VGDLParser(object):
    ...
    def __init__(self, game_str=None, **kwargs):
        ...
        # Pre-define common sprite instances to avoid NameErrors
        self._predefine_common_sprites()

    def _predefine_common_sprites(self):
        """Pre-define common sprite instances to avoid NameErrors"""
        try:
            import pygame
            from vgdl_env.vgdl.core import VGDLSprite
            
            # List of common sprite names that might be referenced before definition
            common_sprites = ['key', 'goal', 'avatar', 'wall', 'hole', 'box', 'mushroom']
            
            for sprite_name in common_sprites:
                if sprite_name not in globals():
                    # Create a sprite class for this instance
                    sprite_class = type(f"{sprite_name.capitalize()}Sprite", (VGDLSprite,), {
                        '__doc__': f"Pre-defined sprite class for {sprite_name}",
                        'color': (150, 150, 150),
                        'update': lambda self, game: None,
                        '__init__': lambda self, pos=None, size=None, color=None, **kwargs: 
                            VGDLSprite.__init__(self, pos or (0, 0), size or (10, 10), color or (150, 150, 150), **kwargs)
                    })
                    
                    # Create a proper rect for the position
                    pos = (0, 0)
                    size = (10, 10)
                    rect = pygame.Rect(pos[0], pos[1], size[0], size[1])
                    
                    # Create an instance with proper rect
                    sprite_instance = sprite_class(pos, size)
                    sprite_instance.rect = rect  # Ensure rect is properly set
                    sprite_instance.stypes = [sprite_name]  # Set stypes to include the sprite name
                    
                    globals()[sprite_name] = sprite_instance
                    print(f"VGDLParser: Pre-defined sprite instance for {sprite_name}")
        except Exception as e:
            print(f"VGDLParser: Failed to pre-define common sprites: {e}")
    ...
    def _eval(self, estr):
        ...
        """
        # List of common VGDL sprite classes that might be referenced in game definitions
        vgdl_sprite_classes = [
            'Immovable', 'MovingAvatar', 'HorizontalAvatar', 'VerticalAvatar', 'FlakAvatar',
            'OrientedAvatar', 'RotatingAvatar', 'RotatingFlippingAvatar', 'NoisyRotatingFlippingAvatar',
            'ShootAvatar', 'AimedAvatar', 'AimedFlakAvatar', 'InertialAvatar', 'MarioAvatar',
            'ClimbingAvatar', 'FrostBiteAvatar', 'Passive', 'Flicker', 'Spreader', 'SpriteProducer',
            'Portal', 'SpawnPoint', 'RandomNPC', 'OrientedSprite', 'Conveyor', 'Missile', 'Switch',
            'OrientedFlicker', 'Walker', 'WalkJumper', 'RandomInertial', 'RandomMissile',
            'ErraticMissile', 'Bomber', 'Chaser', 'Fleeing', 'AStarChaser', 'Resource', 'ResourcePack',
            'Floe', 'FrostbiteIgloo', 'SpriteCounter'
        ]
        
        # List of common VGDL functions that might be referenced in game definitions
        vgdl_functions = [
            'stepBack', 'killSprite', 'killIfTooFast', 'killIfHasMore', 'killIfHasLess', 
            'killIfOtherHasMore', 'killIfOtherHasLess', 'collectResource', 'changeScore', 
            'conveySprite', 'changeResource', 'nothing', 'bounceForward', 'undoAll',
            'reverseDirection', 'attractGaze', 'turnAround', 'transformTo', 'transformToSingleton',
            'wrapAround', 'pullWithIt', 'teleportToExit', 'cloneSprite', 'flipDirection',
            'bounceDirection', 'wallBounce', 'wallStop', 'killIfSlow', 'killIfAlive',
            'collectResourceIfHeld', 'killIfFromAbove', 'killIfFast'
        ]
        
        # First, check if it's a sprite class
        if estr in vgdl_sprite_classes and estr not in globals():
            print(f"VGDLParser: Attempting to ensure sprite class {estr} is defined in globals")
            try:
                # Try to import from various possible locations
                try:
                    # Try to import from vgdl_env.vgdl.ontology
                    import importlib
                    ontology_module = importlib.import_module('vgdl_env.vgdl.ontology')
                    if hasattr(ontology_module, estr):
                        sprite_class = getattr(ontology_module, estr)
                        
                        # Special handling for factory pattern classes like MovingAvatar
                        if estr == 'MovingAvatar':
                            print(f"VGDLParser: Patching {estr} factory class to return instances")
                            # Import VGDLSprite here to avoid circular imports
                            from vgdl_env.vgdl.core import VGDLSprite
                            
                            # Create a patched version of MovingAvatar that returns instances
                            class PatchedMovingAvatar(VGDLSprite):
                                """Patched version of MovingAvatar that returns instances"""
                                color = (1, 87, 155)  # DARKBLUE
                                speed = 1
                                is_avatar = True
                                alternate_keys = False
                                
                                def __init__(self, pos, size=None, color=None, **kwargs):
                                    super(PatchedMovingAvatar, self).__init__(pos, size, color or self.color, **kwargs)
                            ...
                            # Use our patched class instead
                            globals()[estr] = PatchedMovingAvatar
                            print(f"VGDLParser: Added patched {estr} to globals")
                        else:
                            # For non-factory classes, add them directly
                            globals()[estr] = sprite_class
                            print(f"VGDLParser: Added {estr} from vgdl_env.vgdl.ontology to globals")
                    else:
                        raise ImportError(f"{estr} not found in vgdl_env.vgdl.ontology")
                except ImportError:
                    try:
                        # Try to import from vgdl.ontology
                        ontology_module = importlib.import_module('vgdl.ontology')
                        if hasattr(ontology_module, estr):
                            sprite_class = getattr(ontology_module, estr)
                            ...
                            if estr == 'MovingAvatar':
                                ...
                                # For non-factory classes, add them directly
                                globals()[estr] = sprite_class
                                print(f"VGDLParser: Added {estr} from vgdl.ontology to globals")
                        else:
                            raise ImportError(f"{estr} not found in vgdl.ontology")
                    except ImportError:
                        # Define a minimal fallback class if all imports fail
                        from vgdl_env.vgdl.core import VGDLSprite
                        
                        # Create a dynamic class with the name from estr that handles required constructor args
                        fallback_class = type(estr, (VGDLSprite,), {
                            '__doc__': f"Fallback implementation of {estr}",
                            'color': (128, 128, 128),  # GRAY
                            'is_static': True if estr == 'Immovable' else False,
                            'update': lambda self, game: None,
                            '__init__': lambda self, pos=None, size=None, color=None, **kwargs: 
                                VGDLSprite.__init__(self, pos or (0, 0), size, color or (128, 128, 128), **kwargs)
                        })
                        
                        globals()[estr] = fallback_class
                        print(f"VGDLParser: Created fallback {estr} class in global namespace")
            except Exception as e:
                print(f"VGDLParser: Error ensuring sprite class {estr} is defined: {e}")
        
        # Then, check if it's a function
        elif estr in vgdl_functions and estr not in globals():
            print(f"VGDLParser: Attempting to ensure function {estr} is defined in globals")
            try:
                # Try to import from various possible locations
                try:
                    # Try to import from vgdl_env.vgdl.ontology
                    ...
                    ontology_module = importlib.import_module('vgdl_env.vgdl.ontology')
                    if hasattr(ontology_module, estr):
                        func = getattr(ontology_module, estr)
                        globals()[estr] = func
                        print(f"VGDLParser: Added function {estr} from vgdl_env.vgdl.ontology to globals")
                    else:
                        raise ImportError(f"Function {estr} not found in vgdl_env.vgdl.ontology")
                except ImportError:
                    try:
                        # Try to import from vgdl.ontology
                        ontology_module = importlib.import_module('vgdl.ontology')
                        if hasattr(ontology_module, estr):
                            func = getattr(ontology_module, estr)
                            globals()[estr] = func
                            print(f"VGDLParser: Added function {estr} from vgdl.ontology to globals")
                        else:
                            raise ImportError(f"Function {estr} not found in vgdl.ontology")
                    except ImportError:
                        # Define a minimal fallback function if all imports fail
                        if estr == 'stepBack':
                            # Define a minimal stepBack function
                            def stepBack(sprite, partner, game):
                                """ Revert last move. """
                                if hasattr(sprite, 'lastrect'):
                                    sprite.rect = sprite.lastrect
                                return ("stepBack", getattr(sprite, 'ID', 0), getattr(partner, 'ID', 0) if partner else 0)
                            
                            globals()[estr] = stepBack
                            print(f"VGDLParser: Created fallback stepBack function in global namespace")
                        else:
                            # Generic fallback function for other functions
                            def fallback_func(*args, **kwargs):
                                print(f"Fallback implementation of {estr} called")
                                return (estr, args[0].ID if args and hasattr(args[0], 'ID') else 0, 
                                        args[1].ID if len(args) > 1 and hasattr(args[1], 'ID') else 0)
                            
                            globals()[estr] = fallback_func
                            print(f"VGDLParser: Created fallback function {estr} in global namespace")
            except Exception as e:
                print(f"VGDLParser: Error ensuring function {estr} is defined: {e}")
        
        # Import moved to module level
        try:
            return eval(estr)
        except NameError as e:
            print(f"VGDLParser: NameError evaluating '{estr}': {e}")
            # If it's a class name that wasn't in our list, try to handle it
            if estr not in globals() and estr[0].isupper():  # Class names typically start with uppercase
                print(f"VGDLParser: Attempting to create fallback for unknown class {estr}")
                try:
                    from vgdl_env.vgdl.core import VGDLSprite
                    # Create a dynamic fallback class with proper constructor
                    fallback_class = type(estr, (VGDLSprite,), {
                        '__doc__': f"Auto-generated fallback for {estr}",
                        'color': (100, 100, 100),
                        'update': lambda self, game: None,
                        '__init__': lambda self, pos=None, size=None, color=None, **kwargs: 
                            VGDLSprite.__init__(self, pos or (0, 0), size, color or (100, 100, 100), **kwargs)
                    })
                    globals()[estr] = fallback_class
                    print(f"VGDLParser: Created emergency fallback for {estr}")
                    return fallback_class
                except Exception as inner_e:
                    print(f"VGDLParser: Failed to create fallback for {estr}: {inner_e}")
            # Check if it's a sprite instance name (lowercase first letter) from the game definition
            elif estr not in globals() and estr[0].islower() and estr in ['key', 'goal', 'avatar', 'wall', 'hole', 'box', 'mushroom']:
                print(f"VGDLParser: Attempting to create fallback sprite instance for {estr}")
                try:
                    # Use the pre-defined sprite if available
                    if estr in globals():
                        print(f"VGDLParser: Using pre-defined sprite instance for {estr}")
                        return globals()[estr]
                    ...
                    # Create a sprite class for this instance
                    sprite_class = type(f"{estr.capitalize()}Sprite", (VGDLSprite,), {
                        '__doc__': f"Auto-generated sprite class for {estr}",
                        'color': (150, 150, 150),
                        'update': lambda self, game: None,
                        '__init__': lambda self, pos=None, size=None, color=None, **kwargs: 
                            VGDLSprite.__init__(self, pos or (0, 0), size or (10, 10), color or (150, 150, 150), **kwargs)
                    ...
                    sprite_instance = sprite_class(pos, size)
                    sprite_instance.rect = rect  # Ensure rect is properly set
                    sprite_instance.stypes = [estr]  # Set stypes to include the sprite name
                    
                    globals()[estr] = sprite_instance
                    print(f"VGDLParser: Created emergency fallback sprite instance for {estr}")
                    return sprite_instance
                except Exception as inner_e:
                    print(f"VGDLParser: Failed to create fallback sprite instance for {estr}: {inner_e}")
            
            # If it looks like a function name (lowercase first letter), create a fallback function
            elif estr not in globals() and estr[0].islower():
                print(f"VGDLParser: Attempting to create fallback for unknown function {estr}")
                try:
                    # Create a generic fallback function
                    def generic_fallback(*args, **kwargs):
                        print(f"Generic fallback for {estr} called")
                        return (estr, getattr(args[0], 'ID', 0) if args else 0, 
                                getattr(args[1], 'ID', 0) if len(args) > 1 else 0)
                    
                    globals()[estr] = generic_fallback
                    print(f"VGDLParser: Created emergency fallback function for {estr}")
                    return generic_fallback
                except Exception as inner_e:
                    print(f"VGDLParser: Failed to create fallback function for {estr}: {inner_e}")
            raise  # Re-raise the original exception if we couldn't handle it
...
class BasicGame(object):
    ...
    def __init__(self, **kwargs):
        # Import from the centralized vgdl_imports module
        from .ontology import Immovable, MovingAvatar
        from .colors import DARKGRAY, BLACK, GOLD
        ...
        # Initialize actions attribute to fix 'BasicGame' object has no attribute 'actions' error
        self.actions = []
    ...
    def buildLevel(self, lstr):
        ...
        lengths = list(map(len, lines))  # Convert to list to allow multiple iterations
        ...
        # set up resources
        for res_type, (sclass, args, _) in self.sprite_constr.items():  # items() for Python 3 compatibility
    ...
    def _createSprite(self, keys, pos):
        ...
        for key in keys:
            ...
            try:
                ...
                if s is None:
                    raise ValueError("Sprite constructor returned None")
                s.stypes = stypes
                res.append(s)
                ...
                for sprite1 in sprite_list1:
                    ...
                    for collision_index in sprite1.rect.collidelistall(sprite_list2):
                        ...
                        if effect.__name__ == 'changeResource':
                            ...
                            # Add defensive check for None resource
                            if resource is None or resource not in self.sprite_constr:
                                # Skip this effect if resource is None or not found
                                continue
...
# Add VGDLGame as an alias for BasicGame for backward compatibility
VGDLGame = BasicGame
...
# Termination class moved to ontology.py to avoid circular imports
