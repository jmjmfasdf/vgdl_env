...
# Base directories for game resources
BASEDIRS = ['.', 'games', 'games/game_engines/vgdl_competition/games']
import warnings
import weakref
import struct
import array
...
# Import VGDL core components
# These will be set by the lazy import function
VGDLSprite = None
Resource = None
Avatar = None

# Cache for the core module to avoid repeated imports
_core_module = None

def _lazy_import_core():
    """Lazily import core module to break circular imports.
    Returns (VGDLSprite, Resource, Avatar) tuple.
    """
    global VGDLSprite, Resource, Avatar, _core_module
    
    if VGDLSprite is None:
        # Import core module once and cache it
        if _core_module is None:
            import sys
            import importlib
            _core_module = importlib.import_module('vgdl.core')
            
        # Get the classes from the core module
        VGDLSprite = _core_module.VGDLSprite
        Resource = _core_module.Resource
        Avatar = _core_module.Avatar
    
    return VGDLSprite, Resource, Avatar
...
try:
    ...
    triPoints = unitVector = vectNorm = oncePerStep = None
...
try:
    ...
    # Define basic colors if colors module is not available
    WHITE = (255, 255, 255)
    BLACK = (0, 0, 0)
    RED = (255, 0, 0)
    GREEN = (0, 255, 0)
...
# Base directions for movement
BASEDIRS = [UP, DOWN, LEFT, RIGHT]
...
NO_DIR = None  # Define NO_DIR as None for compatibility with imports
...
#     Sprite types
# ---------------------------------------------------------------------
# Define these as None at module level, will be set after core is fully loaded
VGDLSprite = None
Resource = None

# Removed duplicate _lazy_import_core function that only returned two values
...
class Immovable:
    ...
    def __new__(cls, **kwargs):
        # Ensure core classes are imported before creating the instance
        VGDLSprite, _, _ = _lazy_import_core()
        
        # Create a new class that inherits from VGDLSprite
        class _Immovable(VGDLSprite):
            color = cls.color
            is_static = cls.is_static
            
            def __init__(self, **kwargs):
                super(_Immovable, self).__init__(**kwargs)
                
            def update(self, game):
                """Immovable objects don't move, so this method is intentionally empty."""
                pass
                
        # Create and return an instance of the new class
        return _Immovable(**kwargs)
...
class Passive:
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _Passive(VGDLSprite):
            color = cls.color
            
            def __init__(self, **kwargs):
                super(_Passive, self).__init__(**kwargs)
                
            def update(self, game):
                """Passive objects can be moved by other objects but don't move on their own."""
                # Call parent's update if it exists
                if hasattr(super(_Passive, self), 'update'):
                    super(_Passive, self).update(game)
                
        # Create and return an instance of the new class
        return _Passive(**kwargs)
...
class ResourcePack:
    ...
    def __new__(cls, **kwargs):
        ...
        from vgdl.core import Resource
        
        class _ResourcePack(Resource):
            is_static = True
            
            def __init__(self, **kwargs):
                super(_ResourcePack, self).__init__(**kwargs)
                
        return _ResourcePack(**kwargs)
...
class Flicker:
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _Flicker(VGDLSprite):
            color = cls.color
            limit = cls.limit
            
            def __init__(self, **kwargs):
                super(_Flicker, self).__init__(**kwargs)
                self._age = 0
                
            def update(self, game):
                """
                Update the flicker's state. The sprite will be removed after 'limit' updates.
                """
                # Call parent's update if it exists
                if hasattr(super(_Flicker, self), 'update'):
                    super(_Flicker, self).update(game)
                    
                if self._age > self.limit:
                    if hasattr(game, 'killSprite'):
                        game.killSprite(self)
                    elif hasattr(game, 'removeSprite'):
                        game.removeSprite(self)
                self._age += 1
        
            # Create and return an instance of the new class
        return _Flicker(**kwargs)

class Spreader:
    """ 
    A sprite that spreads to its four canonical neighbor positions and replicates itself there
    if those positions are unoccupied.
    """
    spreadprob = 1.0
    color = GREEN
    is_static = True
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _Spreader(VGDLSprite):
            spreadprob = cls.spreadprob
            color = cls.color
            is_static = cls.is_static
            
            def __init__(self, **kwargs):
                super(_Spreader, self).__init__(**kwargs)
                self._age = 0
            
            def update(self, game):
                # Call parent's update if it exists
                if hasattr(super(_Spreader, self), 'update'):
                    super(_Spreader, self).update(game)
                
                # Only spread with a certain probability
                if random.random() > self.spreadprob:
                    return
                
                # Define the four canonical directions (up, right, down, left)
                directions = [(0, -1), (1, 0), (0, 1), (-1, 0)]
                
                for dx, dy in directions:
                    # Calculate new position
                    new_x = self.rect.centerx + dx * self.rect.width
                    new_y = self.rect.centery + dy * self.rect.height
                    
                    # Check if position is occupied
                    position_occupied = False
                    if hasattr(game, 'getSprites'):
                        for sprite in game.getSprites():
                            if hasattr(sprite, 'rect') and sprite.rect.collidepoint(new_x, new_y):
                                position_occupied = True
                                break
                    
                    # If position is free and within game bounds, create a new sprite
                    if not position_occupied and hasattr(game, 'addSprite'):
                        try:
                            new_sprite = self.__class__(**self.__dict__)
                            if hasattr(new_sprite, 'rect'):
                                new_sprite.rect = self.rect.copy()
                                new_sprite.rect.centerx = new_x
                                new_sprite.rect.centery = new_y
                                game.addSprite(new_sprite)
                        except Exception as e:
                            print(f"Error creating new spreader: {e}")
                
                self._age += 1
        
        # Create and return an instance of the new class
        return _Spreader(**kwargs)

class SpriteProducer:
    """
    Superclass for all sprites that may produce other sprites of a specific type ('stype').
    Subclasses should set the 'stype' class variable to the type of sprite they produce.
    """
    stype = None
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _SpriteProducer(VGDLSprite):
            stype = cls.stype
            
            def __init__(self, **kwargs):
                super(_SpriteProducer, self).__init__(**kwargs)
                
            def produce(self, game, position=None, **kwargs):
                """
                Produce a new sprite of type 'stype' at the specified position.
                If no position is provided, use the current sprite's position.
                """
                if self.stype is None:
                    print("Warning: SpriteProducer has no sprite type (stype) defined")
                    return None
                    
                try:
                    # Create a new sprite of the specified type
                    new_sprite = self.stype(**kwargs)
                    
                    # Set the position if provided, otherwise use the current sprite's position
                    if position is not None:
                        new_sprite.rect.x = position[0]
                        new_sprite.rect.y = position[1]
                    else:
                        new_sprite.rect.x = self.rect.x
                        new_sprite.rect.y = self.rect.y
                        
                    return new_sprite
                    
                except Exception as e:
                    print(f"Error in SpriteProducer.produce: {e}")
                    return None
        
        # Create and return an instance of the new class
        return _SpriteProducer(**kwargs)

class Portal:
    """
    A portal that can teleport sprites to another location.
    When a sprite enters the portal, it's teleported to the portal's 'dest' attribute.
    """
    is_static = True
    color = BLUE
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _Portal(VGDLSprite):
            is_static = cls.is_static
            color = cls.color
            
            def __init__(self, **kwargs):
                super(_Portal, self).__init__(**kwargs)
                self.dest = kwargs.get('dest', None)
            
            def update(self, game):
                # Call parent's update if it exists
                if hasattr(super(_Portal, self), 'update'):
                    super(_Portal, self).update(game)
                
                # Check for sprites colliding with this portal
                if hasattr(game, 'getSprites') and self.dest is not None:
                    for sprite in game.getSprites():
                        if (sprite is not self and hasattr(sprite, 'rect') and 
                            hasattr(self, 'rect') and self.rect.colliderect(sprite.rect)):
                            self.teleport(sprite, game)
            
            def teleport(self, sprite, game):
                """Teleport the sprite to the destination coordinates."""
                try:
                    if hasattr(sprite, 'rect') and hasattr(self.dest, 'rect'):
                        # Save the sprite's offset from the destination portal
                        offset_x = sprite.rect.centerx - self.rect.centerx
                        offset_y = sprite.rect.centery - self.rect.centery
                        
                        # Move the sprite to the destination portal's position plus the offset
                        sprite.rect.centerx = self.dest.rect.centerx + offset_x
                        sprite.rect.centery = self.dest.rect.centery + offset_y
                        
                        # Call the sprite's on_teleport method if it exists
                        if hasattr(sprite, 'on_teleport'):
                            sprite.on_teleport(game)
                            
                except Exception as e:
                    print(f"Error in Portal.teleport: {e}")
...
class SpawnPoint:
    """
    A point that can spawn sprites with a certain probability and cooldown.
    Can be configured to spawn a limited total number of sprites.
    """
    prob = 1.0
    total = None
    color = BLACK
    ...
    def __new__(cls, spawnCooldown=None, prob=None, total=None, **kwargs):
        # Ensure core classes are imported before creating the instance
        VGDLSprite, _, _ = _lazy_import_core()
        
        # Create a new class that inherits from SpriteProducer
        class _SpawnPoint(SpriteProducer):
            prob = cls.prob
            total = cls.total
            color = cls.color
            spawnCooldown = cls.spawnCooldown
            is_static = cls.is_static
            
            def __init__(self, spawnCooldown=None, prob=None, total=None, **kwargs):
                # Initialize the parent class
                super(_SpawnPoint, self).__init__(**kwargs)
                
                # Initialize with provided values or keep defaults
                if prob is not None:
                    self.prob = float(prob)
                    self.is_stochastic = (self.prob > 0 and self.prob < 1)
                    
                if spawnCooldown is not None:
                    self.spawnCooldown = int(spawnCooldown)
                    
                if total is not None:
                    self.total = int(total)
                    
                # Internal counter for cooldown
                self.counter = 0
                self.spawned_count = 0
            
            def update(self, game):
                """
                Update the spawn point. Each cooldown cycle, there's a chance to spawn a new sprite.
                If a total spawn limit is set, the spawner will be removed after reaching it.
                """
                # Call parent's update if it exists
                if hasattr(super(_SpawnPoint, self), 'update'):
                    super(_SpawnPoint, self).update(game)
                    
                # Check if we've reached the spawn limit
                if self.total is not None and self.spawned_count >= self.total:
                    if hasattr(game, 'killSprite'):
                        game.killSprite(self)
                    return
                    
                # Check if it's time to spawn
                if self.counter % self.spawnCooldown == 0:
            ...
            def spawn(self, game):
                """
                Spawn a new sprite at this spawn point's position.
                Returns the spawned sprite or None if spawning failed.
                """
                if self.stype is None:
                    print("Warning: SpawnPoint has no sprite type (stype) defined")
                    return None
                    
                try:
                    # Use the parent class's produce method to create the sprite
                    sprite = self.produce(game)
                    
                    if sprite:
                        # Increment the spawn counter
                        self.spawned_count += 1
                        
                        # Add the sprite to the game if it's not already added
                        if hasattr(game, 'addSprite'):
                            game.addSprite(sprite)
                        
                        return sprite
                        
                except Exception as e:
                    print(f"Error in SpawnPoint.spawn: {e}")
                    
                return None
        
        # Create and return an instance of the new class
        return _SpawnPoint(spawnCooldown=spawnCooldown, prob=prob, total=total, **kwargs)
...
class RandomNPC:
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _RandomNPC(VGDLSprite):
            speed = cls.speed
            is_stochastic = cls.is_stochastic
            
            def __init__(self, **kwargs):
                super(_RandomNPC, self).__init__(**kwargs)
                self.actions = [UP, DOWN, LEFT, RIGHT, NO_ACTION]
                
            def update(self, game):
                """Choose a random action and move in that direction."""
                # Call parent's update if it exists
                if hasattr(super(_RandomNPC, self), 'update'):
                    super(_RandomNPC, self).update(game)
                    
                # Choose a random action and move
                action = random.choice(self.actions)
                self.physics.activeMovement(self, action, self.speed)
        
        # Create and return an instance of the new class
        return _RandomNPC(**kwargs)
...
class OrientedSprite:
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a new class that inherits from VGDLSprite
        class _OrientedSprite(VGDLSprite):
            draw_arrow = cls.draw_arrow
            orientation = cls.orientation
            
            def __init__(self, **kwargs):
                super(_OrientedSprite, self).__init__(**kwargs)
                
            def _draw(self, game, screen, rect):
                """ With a triangle that shows the orientation. """
                from pygame import draw
                
                # Call parent's draw method if it exists
                if hasattr(super(_OrientedSprite, self), '_draw'):
                    super(_OrientedSprite, self)._draw(game, screen, rect)
                ...
                if self.draw_arrow:
                    ...
                    # Define arrow points based on orientation
                    if self.orientation == UP:
                        points = [(center[0], rect.top), (rect.left, rect.bottom), (rect.right, rect.bottom)]
                    elif self.orientation == RIGHT:
                        points = [(rect.right, center[1]), (rect.left, rect.top), (rect.left, rect.bottom)]
                    elif self.orientation == DOWN:
                        points = [(center[0], rect.bottom), (rect.left, rect.top), (rect.right, rect.top)]
                    elif self.orientation == LEFT:
                        points = [(rect.left, center[1]), (rect.right, rect.top), (rect.right, rect.bottom)]
                        
                    if points:
                        draw.polygon(screen, (255, 0, 0), points)
        
        # Create and return an instance of the new class
        return _OrientedSprite(**kwargs)
...
class Conveyor:
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from OrientedSprite
        class _Conveyor(OrientedSprite):
            is_static = cls.is_static
            color = cls.color
            ...
            def __init__(self, **kwargs):
                ...
                super(_Conveyor, self).__init__(**kwargs)
        
        # Create and return an instance of the new class
        return _Conveyor(**kwargs)
...
class Missile:
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from OrientedSprite
        class _Missile(OrientedSprite):
            speed = cls.speed
            color = cls.color
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_Missile, self).__init__(**kwargs)
                
            def update(self, game):
                """Move the missile in its current direction."""
                # Call parent's update if it exists
                if hasattr(super(_Missile, self), 'update'):
                    super(_Missile, self).update(game)
                    
                # Move in the current direction
                if hasattr(self, 'physics') and hasattr(self, 'orientation'):
                    self.physics.activeMovement(self, self.orientation, self.speed)
        
        # Create and return an instance of the new class
        return _Missile(**kwargs)
...
class Switch:
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from VGDLSprite
        class _Switch(VGDLSprite):
            activated = cls.activated
            wait_for_release = cls.wait_for_release
            can_switch = cls.can_switch
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_Switch, self).__init__(**kwargs)
                self.activated = kwargs.get('activated', self.activated)
                self.wait_for_release = kwargs.get('wait_for_release', self.wait_for_release)
                self.can_switch = kwargs.get('can_switch', self.can_switch)
                
            def update(self, game):
                """Update the switch's state."""
                # Call parent's update if it exists
                if hasattr(super(_Switch, self), 'update'):
                    super(_Switch, self).update(game)
...
class OrientedFlicker:
    """
    A flickering sprite that maintains orientation.
    Combines functionality of Flicker and OrientedSprite.
    """
    draw_arrow = True
    speed = 0
    ...
    def __new__(cls, **kwargs):
        ...
        # Create a dynamic class that inherits from both Flicker and OrientedSprite
        class _OrientedFlicker(Flicker, OrientedSprite):
            draw_arrow = cls.draw_arrow
            speed = cls.speed
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_OrientedFlicker, self).__init__(**kwargs)
        
        # Create and return an instance of the new class
        return _OrientedFlicker(**kwargs)

class Walker:
    """ Keep moving in the current horizontal direction. If stopped, pick one randomly. """
    airsteering = False
    is_stochastic = True
    speed = 1  # Default speed for Walker
    
    def __new__(cls, **kwargs):
        # Ensure core classes are imported before creating the instance
        VGDLSprite, _, _ = _lazy_import_core()
        
        # Dynamically create a class that inherits from Missile
        class _Walker(Missile):
            airsteering = cls.airsteering
            is_stochastic = cls.is_stochastic
            speed = cls.speed
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_Walker, self).__init__(**kwargs)
                self.orientation = RIGHT  # Default starting direction
                
            def update(self, game):
                # If we're not moving, pick a random direction
                if not hasattr(self, 'orientation') or self.orientation is None:
                    self.orientation = RIGHT
                    
                # If we're not moving, pick a random direction
                if not hasattr(self, 'lastmove') or self.lastmove is None or self.lastmove <= 0:
                    self.orientation = random.choice([LEFT, RIGHT])
                    
                # Call parent's update
                super(_Walker, self).update(game)
        
        # Create and return an instance of the new class
        return _Walker(**kwargs)

class WalkJumper:
    """
    A walker that can randomly jump when it hits an obstacle.
    Combines Walker behavior with occasional jumping.
    """
    prob = 0.1
    strength = 10
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from Walker
        class _WalkJumper(Walker):
            prob = cls.prob
            strength = cls.strength
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_WalkJumper, self).__init__(**kwargs)
                
            def update(self, game):
                """Update the walker's state, with a chance to jump when hitting obstacles."""
                # Check if we're stuck (not moving in x direction)
                if hasattr(self, 'lastdirection') and self.lastdirection[0] == 0:
                    # Random chance to jump
                    if random.random() < self.prob:
                        if hasattr(self, 'physics') and hasattr(self, 'strength'):
                            self.physics.activeMovement(self, (0, -self.strength), self.strength)
                
                # Call parent's update
                super(_WalkJumper, self).update(game)
        
        # Create and return an instance of the new class
        return _WalkJumper(**kwargs)

class RandomInertial:
    """
    A sprite that moves randomly with physics-based movement.
    Combines OrientedSprite and RandomNPC behaviors with continuous physics.
    """
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from both OrientedSprite and RandomNPC
        class _RandomInertial(OrientedSprite, RandomNPC):
            physicstype = cls.physicstype
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_RandomInertial, self).__init__(**kwargs)
                
                # Ensure physics is set up correctly
                if hasattr(self, 'physics') and self.physics is None:
                    self.physics = self.physicstype()
        
        # Create and return an instance of the new class
        return _RandomInertial(**kwargs)

class RandomMissile:
    """
    A missile that moves in a random direction with random speed.
    Inherits from Missile but overrides the orientation and speed.
    """
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from Missile
        class _RandomMissile(Missile):
            def __init__(self, **kwargs):
                # Set random orientation and speed if not specified
                if 'orientation' not in kwargs:
                    kwargs['orientation'] = random.choice(BASEDIRS)
                if 'speed' not in kwargs:
                    kwargs['speed'] = random.choice([0.1, 0.2, 0.4])
                    
                # Call the parent class's __init__ with the updated kwargs
                super(_RandomMissile, self).__init__(**kwargs)
        
        # Create and return an instance of the new class
        return _RandomMissile(**kwargs)

class ErraticMissile:
    """
    A missile that occasionally changes direction.
    Inherits from Missile but adds erratic movement behavior.
    """
    prob = 0.1  # Probability of changing direction each update
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from Missile
        class _ErraticMissile(Missile):
            prob = cls.prob
            
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_ErraticMissile, self).__init__(**kwargs)
                
            def update(self, game):
                """Update the missile's position, with a chance to change direction."""
                # Random chance to change direction
                if random.random() < self.prob:
                    self.orientation = random.choice(BASEDIRS)
                    
                # Call parent's update
                if hasattr(super(_ErraticMissile, self), 'update'):
                    super(_ErraticMissile, self).update(game)
        
        # Create and return an instance of the new class
        return _ErraticMissile(**kwargs)



class Bomber:
    """
    A sprite that drops bombs as it moves.
    Combines SpawnPoint and Missile behaviors.
    """
    color = ORANGE
    speed = 0
    prob = 0.01
    ...
    def __new__(cls, **kwargs):
        ...
        # Dynamically create a class that inherits from both SpawnPoint and Missile
        class _Bomber(SpawnPoint, Missile):
            color = cls.color
            speed = cls.speed
            prob = cls.prob
            ...
            def __init__(self, **kwargs):
                # Call the parent class's __init__
                super(_Bomber, self).__init__(**kwargs)
            ...
            def update(self, game):
                """Update the bomber's position and drop bombs."""
                # Update movement using Missile's update
                if hasattr(super(_Bomber, self), 'update'):
                    super(_Bomber, self).update(game)
                    
                # Handle bomb dropping
                self.lastmove -= 1
                if self.lastmove <= 0 and random.random() < self.prob:
                    self.lastmove = self.cooldown
                    self.spawn(game)
        
        # Create and return an instance of the new class
        return _Bomber(**kwargs)
...
class Fleeing(Chaser):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(Fleeing, self).__init__(**kwargs)
...
class AStarChaser:
    ...
    def __new__(cls, **kwargs):
        # Import VGDLSprite here to avoid circular imports
        VGDLSprite, _, _ = _lazy_import_core()
        
        class _AStarChaser(VGDLSprite):
            stype = cls.stype
            speed = cls.speed
            fleeing = cls.fleeing
            drawpath = cls.drawpath
            walkableTiles = cls.walkableTiles
            neighborNodes = cls.neighborNodes
            
            def __init__(self, **kwargs):
                super(_AStarChaser, self).__init__(**kwargs)
    path = []
    next_move = None
    last_move = None
...
# ---------------------------------------------------------------------
# Avatar class is now defined in core.py and imported lazily
...
class MovingAvatar:
    ...
    def __new__(cls, **kwargs):
        ...
        class _MovingAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            
            def __init__(self, **kwargs):
                super(_MovingAvatar, self).__init__(**kwargs)
...
class HorizontalAvatar:
    """ Avatar that can only move horizontally (left/right). """
    color = WHITE
    speed = 1
    is_avatar = True
    alternate_keys = False
    ...
    def __new__(cls, **kwargs):
        ...
        class _HorizontalAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            ...
            def __init__(self, **kwargs):
                super(_HorizontalAvatar, self).__init__(**kwargs)
            ...
            def update(self, game):
                from pygame.locals import K_LEFT, K_RIGHT
                from ontology import LEFT, RIGHT

                action = game.get_actions()[0]
                if action == K_LEFT:
                    self.physics.activeMovement(self, LEFT)
                elif action == K_RIGHT:
                    self.physics.activeMovement(self, RIGHT)
        
        return _HorizontalAvatar(**kwargs)

class VerticalAvatar:
    """ Avatar that can only move vertically (up/down). """
    color = WHITE
    speed = 1
    is_avatar = True
    alternate_keys = False
    ...
    def __new__(cls, **kwargs):
        ...
        class _VerticalAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            ...
            def __init__(self, **kwargs):
                super(_VerticalAvatar, self).__init__(**kwargs)
            ...
            def update(self, game):
                from pygame.locals import K_UP, K_DOWN
                from ontology import UP, DOWN

                action = game.get_actions()[0]
                if action == K_UP:
                    self.physics.activeMovement(self, UP)
                elif action == K_DOWN:
                    self.physics.activeMovement(self, DOWN)
        
        return _VerticalAvatar(**kwargs)

class FlakAvatar:
    """ Avatar that can move horizontally and shoot by pressing space. """
    color = GREEN
    speed = 1
    is_avatar = True
    alternate_keys = False
    ...
    def __new__(cls, **kwargs):
        ...
        class _FlakAvatar(VGDLSprite):
            ...
            def __init__(self, **kwargs):
                super(_FlakAvatar, self).__init__(**kwargs)
            ...
            def update(self, game):
                ...
                # Handle movement (copied from HorizontalAvatar)
                action = game.get_actions()[0]
                if action == K_LEFT:
                    self.physics.activeMovement(self, LEFT)
                elif action == K_RIGHT:
            ...
            def _shoot(self, game):
                from pygame.locals import K_SPACE
                if hasattr(self, 'stype') and self.stype and game.keystate[K_SPACE]:
                    spawn = game._createSprite([self.stype], (self.rect.left, self.rect.top))
        
        return _FlakAvatar(**kwargs)
...
class OrientedAvatar:
    ...
    def __new__(cls, **kwargs):
        ...
        class _OrientedAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            ...
            def __init__(self, **kwargs):
                super(_OrientedAvatar, self).__init__(**kwargs)
            ...
            def _readAction(self, game):
                ...
                if not actions:
                    return None
                return actions[0]  # Return first pressed key
        ...
        return _OrientedAvatar(**kwargs)

class RotatingAvatar:
    """ Avatar that rotates with each move. """
    color = WHITE
    speed = 0
    is_avatar = True
    alternate_keys = False
    ...
    def __new__(cls, **kwargs):
        ...
        class _RotatingAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            draw_arrow = cls.draw_arrow
            orientation = (1, 0)  # Default orientation (right)
            
            def __init__(self, **kwargs):
                super(_RotatingAvatar, self).__init__(**kwargs)
                self.BASEDIRS = [
                    (0, -1),  # UP
                    (1, -1),  # UP-RIGHT
                    (1, 0),   # RIGHT
            ...
            def _readMultiActions(self, game):
                from ontology import UP, DOWN, LEFT, RIGHT
                actions = []
                key_actions = self.declare_possible_actions()
                for action_name, key in key_actions.items():
                    if game.keystate.get(key, False):
                        actions.append(locals()[action_name])
                return actions
            ...
            def update(self, game):
                ...
                if UP in actions:
                    ...
                    self.speed = -1
                if LEFT in actions:
                    i = self.BASEDIRS.index(tuple(self.orientation))
                    self.orientation = self.BASEDIRS[(i + 1) % len(self.BASEDIRS)]
        ...
        return _RotatingAvatar(**kwargs)

class RotatingFlippingAvatar:
    """ Avatar that rotates with each move, and can flip between two orientations. """
    color = WHITE
    speed = 0
    is_avatar = True
    alternate_keys = False
    draw_arrow = True
    noiseLevel = 0  # Noise level for random actions
    
    def __new__(cls, **kwargs):
        VGDLSprite, _, _ = _lazy_import_core()
        
        class _RotatingFlippingAvatar(VGDLSprite):
            color = cls.color
            speed = cls.speed
            is_avatar = cls.is_avatar
            alternate_keys = cls.alternate_keys
            draw_arrow = cls.draw_arrow
            noiseLevel = cls.noiseLevel
            orientation = (1, 0)  # Default orientation (right)
            
            def __init__(self, **kwargs):
                super(_RotatingFlippingAvatar, self).__init__(**kwargs)
                self.BASEDIRS = [
                    (0, -1),  # UP
                    (1, -1),  # UP-RIGHT
                    (1, 0),   # RIGHT
            ...
            def _readMultiActions(self, game):
                ...
                # Check for noise
                if self.noiseLevel > 0 and random.random() < self.noiseLevel * 4:
                    return [random.choice([UP, DOWN, LEFT, RIGHT])]
                
                # Regular action reading
                for action_name, key in key_actions.items():
                    if game.keystate.get(key, False):
                        actions.append(locals()[action_name])
                return actions
            ...
            def update(self, game):
                ...
                if UP in actions:
                    ...
                    # Rotate 180 degrees on DOWN
                    i = self.BASEDIRS.index(tuple(self.orientation))
                    self.orientation = self.BASEDIRS[(i + 4) % len(self.BASEDIRS)]
                
                # Handle LEFT/RIGHT rotation if not moving
                if LEFT in actions:
                    i = self.BASEDIRS.index(tuple(self.orientation))
                    self.orientation = self.BASEDIRS[(i + 1) % len(self.BASEDIRS)]
                elif RIGHT in actions:
        ...
        return _RotatingFlippingAvatar(**kwargs)
...
class NoisyRotatingFlippingAvatar(RotatingFlippingAvatar):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(NoisyRotatingFlippingAvatar, self).__init__(**kwargs)
    noiseLevel = 0.1

class ShootAvatar(OrientedAvatar, SpriteProducer):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(ShootAvatar, self).__init__(**kwargs)
    ...
    def _reduceAmmo(self):
        if self.ammo != None and self.ammo in self.resources:
...
class AimedAvatar(ShootAvatar):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(AimedAvatar, self).__init__(**kwargs)
...
class InertialAvatar(OrientedAvatar):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(InertialAvatar, self).__init__(**kwargs)
    speed = 1
    physicstype = ContinuousPhysics
    def update(self, game):
...
class MarioAvatar(InertialAvatar):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(MarioAvatar, self).__init__(**kwargs)
...
class ClimbingAvatar(MarioAvatar, MovingAvatar):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(ClimbingAvatar, self).__init__(**kwargs)
    climbing = False
...
class FrostbiteIgloo(SpawnPoint, Switch):
    def __init__(self, **kwargs):
        VGDLSprite, _ = _lazy_import_core()
        super(FrostbiteIgloo, self).__init__(**kwargs)
