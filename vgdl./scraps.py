	...
	def makeActionSet(self, n_samples):
		...
		# Ensure numActions is positive to avoid dirichlet error
		numActions = max(1, numActions)
		...
		if partition_sum > 0:  # Avoid division by zero
			partition = [p/partition_sum for p in partition]
		else:  # If all probabilities are zero, use uniform distribution
			partition = [1.0/numActions] * numActions
		# Ensure actionKeys is not empty
		actionKeys = list(range(max(1, numActions)))
		outList = [self.actionDict[np.random.choice(actionKeys, p=partition)] for _ in range(max(1, n_samples))]
		return outList
